...
**Starting point.** The welcome tour asks how your first partner starts, and you can change it later in the settings:
...
![Hovering over Dewey shows a status window: name, level, type, stage, an EXP bar, progress to the next stage, mood, today's writing and a reminder of what clicks do](docs/hud.png)
...
| Hover | A status window with level, EXP, progress to the next stage and today's writing |
| Click a bubble | Skips to the next bubble, or jumps to the badge, quest or wardrobe tab it's about |

Speech bubbles look like a game's dialogue box, with your partner's name on a tab and a blinking ▼ once the line has finished typing. Clicks on the transparent parts of the pet go through to whatever is behind it. When XP comes in, `+12 XP` floats up over its head.
...
- **29 badges**: totals for characters, links and notes, a hub note with 20 backlinks, 3,000 characters in a day, streaks, night owl and early bird, marathons, meals, breaks, petting, quests, levels, stages, and meeting and raising friends. On first run, badges your existing notes already earned are awarded quietly all at once.
...
**Updating from 0.1.** Your pet, its XP, badges and items are all kept. On the first start after updating, the plugin shows the partner picker once: whichever friend you choose takes over everything your pet had grown so far. Close it without choosing and your pet simply becomes Inky.
...
```bash
node tests/run.js                          # logic tests without Obsidian (counting, ledger, growth, friends, quests, badges, moods, drawing, both languages)
node scripts/demo-vault.js <folder>        # a demo vault on first run (welcome tour and partner picker)
node scripts/demo-vault.js <folder> --seeded --partner=purrl   # a partner raised for about three weeks, with three friends in the dex
...
노트를 쓸수록 자라는 도트 친구예요. 처음에 다섯 친구 중 하나를 파트너로 골라요.
...
- 글자를 쓰고(20자 = 1XP), 링크를 걸고(5XP), 새 노트를 만들면(15XP) 경험치가 쌓여 알에서 어른까지 자라요. 친구마다 특기만큼 더 받아요.
- 편집 화면 아래를 산책하다가 글을 쓰면 그 자리에 앉아 같이 써요. 잉키는 공책, 냥타래는 노트북, 새록이는 체크리스트, 모닥이는 두루마리, 듀이는 두꺼운 책을 펴요.
- 마우스를 올리면 게임 같은 상태 창(레벨·EXP·진화까지 남은 정도·오늘 쓴 양)이 떠요. 말풍선에는 이름표가 붙고, 다 말하면 ▼ 가 깜빡여요.
...
- 0.1 에서 업데이트하면 파트너 고르기 창이 한 번 떠요. 고른 친구가 지금까지 키운 성장을 그대로 이어받아요.
